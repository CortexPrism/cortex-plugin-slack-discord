...
function checkSlackToken(): string | null {
  if (!config.slackBotToken) {
    return 'Slack bot token not configured. Set slackBotToken in plugin settings.';
  }
  return null;
}
...
function checkDiscordToken(): string | null {
  if (!config.discordBotToken) {
    return 'Discord bot token not configured. Set discordBotToken in plugin settings.';
  }
  return null;
}
  ...
  definition: {
    ...
    params: [
      ...
      },
      {
        name: 'blocks',
        type: 'string',
        description: 'JSON string of Slack Block Kit blocks',
        required: false,
      },
    ],
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (args.blocks && typeof args.blocks === 'string') {
        try {
          body.blocks = JSON.parse(args.blocks);
        } catch {
          return {
            toolName,
            success: false,
...
const slackListChannels: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const result = await slackApi<{ channels: Array<Record<string, unknown>> }>(
        'conversations.list',
        { types, limit: 200 },
      );
...
const slackAddReaction: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      if (
        !args.channel || !args.timestamp || !args.emoji ||
        typeof args.channel !== 'string' || typeof args.timestamp !== 'string' ||
  ...
  definition: {
    ...
    params: [
      ...
      { name: 'content', type: 'string', description: 'Message text', required: true },
      {
        name: 'embeds',
        type: 'string',
        description: 'JSON string of Discord embed objects',
        required: false,
      },
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (args.embeds && typeof args.embeds === 'string') {
        try {
          body.embeds = JSON.parse(args.embeds);
        } catch {
          return {
            toolName,
            success: false,
    ...
    params: [
      {
        name: 'platform',
        type: 'string',
        description: 'Platform(s) to send to',
        required: false,
        enum: ['slack', 'discord', 'both'],
      },
      {
        name: 'slack_channel',
        type: 'string',
      ...
      },
      {
        ...
        type: 'string',
        description: 'Discord channel override',
        required: false,
      },
      {
        name: 'custom_message',
  ...
  execute: async (args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (platform === 'discord' || platform === 'both') {
        if (config.discordBotToken) {
          try {
            const discordChannel = (args.discord_channel_id as string) ||
              config.defaultDiscordChannelId;
            if (discordChannel) {
              await discordApi(`/channels/${discordChannel}/messages`, {
                method: 'POST',
                body: JSON.stringify({ content: standupText }),
              });
...
const webhookListenStatus: Tool = {
  ...
  execute: async (_args: Record<string, unknown>, _ctx: PluginContext): Promise<ToolCallResult> => {
    ...
    return {
      toolName: 'webhook_listen_status',
      success: true,
...
export async function onLoad(ctx: PluginContext): Promise<void> {
  ...
  if (config.slackBotToken) {
    ctx.logger.info('[cortex-plugin-slack-discord] Slack integration enabled');
  }
  if (config.discordBotToken) {
    ctx.logger.info('[cortex-plugin-slack-discord] Discord integration enabled');
  }
}
